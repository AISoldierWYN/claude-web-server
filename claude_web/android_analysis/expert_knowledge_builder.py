...
def build_generation_prompt(module: Dict[str, Any], subcategories: List[Dict[str, Any]]) -> str:
    ...
    if not sub_lines:
        sub_lines.append('- unknown: 暂未定义小类')
    fields = ', '.join(EVIDENCE_TEMPLATE_FIELDS)
    return f"""# Android 证据模板生成任务

请为 `{title}` (`{module_id}`) 生成 Android 问题分析证据模板。
目标是阅读真实源码日志调用，把可单行确认含义的日志转成 `.claude-web/android-analysis/evidence_templates.csv` 和 `evidence_templates.jsonl`。

## 模块说明
{module.get('description') or '(暂无描述)'}
...
## 要求
- 只提取源码中真实存在的日志、状态字段或错误码。
- `regex` 必须尽量绑定稳定的 `TAG + message`，避免纯语义关键词。
- 前置工作流只放单行即可确定含义的证据，不要写需要多行堆栈或上下文推理的规则。
- 复杂 trace、meminfo、ANR/tombstone 线索只写入模块 skill 或交给 Deep 阶段分析。
- `code_location` 写清类名、方法名或文件路径。
- `meaning` 用中文说明命中后代表什么状态或异常。
- 如果使用 `$package_name` 等占位符，必须在 `parameters` 中声明。

生成后请保证 CSV/JSONL 可被校验器读取，并补充模块 skill 中的排查步骤。
"""
        ...
        return [
            {
                ...
                'module_id': module_id,
                'title': '未分类问题',
                'description': '尚未拆分具体小类，可先让 Claude 阅读源码后补充。',
                'aliases': [],
            }
        ]
    out = []
...
def _knowledge_readme(module: Dict[str, Any], subcategories: List[Dict[str, Any]]) -> str:
    ...
    return f"""# {module.get('title')} Android 分析知识包

这是 Claude Web Server Android 专家工作台使用的项目知识包，用于维护模块、小类、证据模板、经验日志和历史案例。

## 维护流程
1. 先确认 `module.json` 和 `subcategories.json`。
2. 使用 `generation_prompt.md` 引导 Claude 阅读源码并填写 `evidence_templates.csv` / `evidence_templates.jsonl`。
3. 人工审阅每条 `regex`，确保它绑定真实日志、状态字段或错误码。
4. 用 CSV/XLSX 与 JSONL 互转工具保持格式一致。
5. 将稳定经验写入 `experience_logs.jsonl`，确认后的历史案例写入 `cases/case_cards.jsonl`。
...
## 目标
分析 `{title}` (`{module_id}`) 相关 Android 问题时，先基于精确证据还原现场，再结合源码、完整日志和项目指南做专家式判断。

## 问题小类
{sub_lines}

## 分析步骤
1. 先阅读前置工作流给出的分类、参数解析、证据模板选择和案例召回结果。
2. 优先检查 `.claude-web/android-analysis/evidence_templates.jsonl` 命中的精确日志。
3. 对照项目中的 `CLAUDE.md` / `AGENTS.md` 理解模块约定。
4. 按 `module.json.source_roots` 指定的范围阅读相关源码。
5. 如果证据不支持前置判断，应明确推翻并说明原因。

## 注意
- 不要只靠模糊 grep 下根因结论，要把日志原文、代码位置和状态链串起来。
- Deep 阶段可以推翻前置结论。
- 非本模块问题要给出接力领域和继续排查入口。
"""
