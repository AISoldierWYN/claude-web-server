...
def build_batch_xml_state_generation_prompt(
    ...
    categories = '\n'.join(category_lines)
    return f"""请批量生成 Android 状态文件 XML/SP 证据模板
模块：
...
要求：
1. 只生成 XML/SharedPreferences/Settings 状态文件模板，不要生成 logcat 模板。
2. 每条模板的 `subcategory_id` 必须来自候选项。
3. 优先选择能直接说明状态机、开关、结果码或持久化事实的 key。
4. `path_patterns` 要限定到合理文件范围，例如 `(?i)(shared_prefs|sp).*user_state.*\\.xml$`。
5. `key_regex` 要尽量精确到完整 key，避免过宽泛的模糊 key。
6. `value_regex` 只在值域稳定时填写，例如 `^(0|1|2)$`、`^true$`；未知可留空。
7. `value_source` 默认 `shared_prefs_value`，如果是 Settings 或普通 XML 可写 `settings_value`。
8. `meaning` 用中文解释该 key 命中后代表什么业务状态。
9. `next_steps` 写清 Deep 阶段应结合哪些日志、源码或其它状态 key。
10. 如果候选只是 Intent extra、UI 文案或临时变量，不要生成 XML/SP 模板，写到 `{notes_file_name}` 说明原因。

必须创建这些文件：
- `{draft_file_name}`：JSONL，每行一个 JSON 对象，不要 Markdown 包裹或 CSV header。
- `{notes_file_name}`：Markdown，记录舍弃候选和人工审阅建议。
"""
