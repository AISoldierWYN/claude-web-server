...
def build_prefiltered_generation_prompt(
    ...
    return f"""???? Android ??????????????
???{module.get('title') or module.get('id')} ({module.get('id')})
???{subcategory.get('title')} ({subcategory.get('id')})
...
??????????
- `{draft_file_name}`??? JSONL??? CSV header????? JSON object?
- `{notes_file_name}`??????????/?????
...
?????
1. ?????????????????????????????
2. `subcategory_id` ????????? `subcategory_id`?
3. `code_location` ??????????? `candidate.path:candidate.line`?
4. `regex` ????????? `candidate.tag + ": " + candidate.message`?
5. ???????? 2-6 ??????????????????????????????????
6. `meaning` ??????????`next_steps` ????????????
7. ??? Markdown ???`{draft_file_name}` ??? JSONL????? JSON ???
8. `{notes_file_name}` ? Markdown ????????/??????
9. ?????`module_id={module.get('id')}`?`profile=functional`?`log_type=android_log`?`enabled=true`?
